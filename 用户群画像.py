...
import pandas as pd
import operator
data=pd.read_table(r'C:\Users\Administrator\Desktop\test10000.txt',delimiter='|',encoding='utf-8',
...
data.columns=['Userid','mobile','imsi','imei','name','age','sex','status','mobile_reg_time',
                      ...
                      'tencent_tv','migu_tv','leeco','cmcc','pptv','other_tv','realty','automobile','traffic_violation',
                      'car_maintenance','car_news','network_car','gps','carer_app','traffic_information','game','read',
                      'Ifeng_read','mobile_read','news_affairs','other_read','finance','financial_management','stock',
                      'Securities','bank','gold','music','migu_music','music_package','pop_music','crbt','music_activities',
                      'sports','cartoon','merchant_travel','education','parent_child_education','growth_plan','family_books',
                      'education_publicity','health_education','house','community','real_estate','cadres','police','civil_servant',
...
def get_age():
    ...
    result={}
    dict={}
    dict['0-15岁']=data_age[(data_age.index>=0)&(data_age.index<15)].sum()
    dict['15-20岁'] = data_age[(data_age.index >=15) & (data_age.index<20)].sum()
    dict['20-25岁'] = data_age[(data_age.index >=20) & (data_age.index<25)].sum()
    dict['25-30岁'] = data_age[(data_age.index >=25) & (data_age.index<30)].sum()
    dict['30-35岁'] = data_age[(data_age.index >=30) & (data_age.index<35)].sum()
    dict['35-40岁'] = data_age[(data_age.index >=35) & (data_age.index<40)].sum()
    dict['40-45岁'] = data_age[(data_age.index >=40) & (data_age.index<45)].sum()
    dict['45-50岁'] = data_age[(data_age.index >=45) & (data_age.index<50)].sum()
    dict['50-55岁'] = data_age[(data_age.index >=50) & (data_age.index<55)].sum()
    dict['55-60岁'] = data_age[(data_age.index >=55) & (data_age.index<60)].sum()
    dict['60岁以上'] = data_age[(data_age.index >=60)].sum()
...
def get_netlong():
    ...
    dict={}
    dict['0-3月'] = data_result[(data_result.index >= 0) & (data_result.index < 3)].sum()
    dict['3-6月'] = data_result[(data_result.index >= 3) & (data_result.index < 6)].sum()
    dict['6-12月'] = data_result[(data_result.index>= 6) & (data_result.index < 12)].sum()
    dict['12-24月'] = data_result[(data_result.index>= 12) & (data_result.index < 24)].sum()
    dict['24-36月'] = data_result[(data_result.index>= 24) & (data_result.index < 36)].sum()
    dict['36月以上'] =data_result[data_result.index>=36].sum()
    result={}
    result['MainClass'] = {'在网时长_MainClass_Range': {'ClassifyValue': dict}}
    result['MainClassTotal'] = {'在网时长汇总': data.age.count()}
    return result
def get_mobilecard_brand():
